...
public interface StompServerInter<T> extends Closeable {
     ...
     */
    public static <T> StompBaseServer<T>  threadPerClient(
            int port,
        ...
        return new StompBaseServer<T>(port, protocolFactory, encoderDecoderFactory) {
            @Override
            protected void execute(TPCConnectionHandler<T>  handler) {
